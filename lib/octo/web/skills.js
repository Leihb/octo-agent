...
const SkillAC = (() => {
  ...
  function _initDOMBindings() {
    ...
    $("user-input").addEventListener("keydown", e => {
      const input = $("user-input");
      const historyKey = Sessions.activeId ? "octo_cmd_history:" + Sessions.activeId : "octo_cmd_history";
      const history = JSON.parse(localStorage.getItem(historyKey) || "[]");

      // Command history: ArrowUp / ArrowDown (only when cursor is at start/end or input is empty)
      if ((e.key === "ArrowUp" || e.key === "ArrowDown") && history.length > 0) {
        ...
        // Allow history nav if: empty input, or cursor at start for Up / at end for Down
        if (isEmpty || (e.key === "ArrowUp" && isAtStart) || (e.key === "ArrowDown" && isAtEnd)) {
          // Don't interfere with skill autocomplete dropdown
          if (_visible) return;
          ...
          if (e.key === "ArrowUp") {
            if (_historyIndex < history.length - 1) {
              _historyIndex++;
              input.value = history[history.length - 1 - _historyIndex];
            }
          } else {
            if (_historyIndex > 0) {
              _historyIndex--;
          ...
          // Move cursor to end and trigger auto-resize
      ...
      // Reset history index on any other key (except arrows handled above)
